...
const SurveyListing = ({surveys}:{surveys:Surveys}) => {
  ...
  return (
    ...
    <Center py="3" px="2" w="100%" alignItems="flex-end">
      {surveys?.length > 4 && <Button p="1" onPress={()=> push(`/${event.url}/surveys`)} _hover={{ bg: 'transparent', _text: { color: 'primary.500' }, _icon: { color: 'primary.500' } }} bg="transparent" width={'auto'} rightIcon={<Icon as={SimpleLineIcons} name="arrow-right" size="sm" />}>
        See all
      </Button>}
    </Center>
  </Container>
