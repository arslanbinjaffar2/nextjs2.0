  ...
  return (
    <Center zIndex={9999} position={'relative'} maxW="100%" w="100%" mb="0">
      <Box zIndex={9999} position={'relative'} mb="3" py="3" px="4" w="100%">
        ...
        <DateTimePicker label="Date and Time" showtime={`HH:mm:ss`} showdate={'DD-MM-YYYY'} value={formData[question.id]?.answer ?? ''} onChange={(currentDate:any)=>{updateFormData(question.id, question.question_type, currentDate._isAMomentObject !== undefined && currentDate._isAMomentObject === true ? moment(currentDate).format("DD-MM-YYYY HH:mm:ss") : '')}} />
      </Box>
