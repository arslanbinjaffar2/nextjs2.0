  ...
  return (
    <Center ref={refElement} maxW="100%" w="100%" mb="0" zIndex={9}>
      <Box zIndex={9999} position={'relative'} mb="3" py="3" px="4" w="100%">
        ...
        <DateTimePicker
        readOnly={ (canChangeAnswer !== undefined && canChangeAnswer == 0) ? true : false } label={"DD-MM-YYYY"} showdate={GENERAL_DATE_FORMAT} initialValue={formData[question.id]?.answer} onChange={(currentDate:any)=>{updateFormData(question.id, question.question_type, currentDate._isAMomentObject !== undefined && currentDate._isAMomentObject === true ? moment(currentDate).format(GENERAL_DATE_FORMAT) : '')}} />
      </Box>
