          ...
          <AlertDialog.Content nativeID="ebs-alert-dialog" bg={'primary.box'}>
            <AlertDialog.Header borderColor={'primary.bdColor'} bg={'transparent'} fontWeight={600}>
              <HStack  space="0" alignItems="center">
               ...
               <Text pl={3} w={'calc(100% - 80px)'} fontSize="md" fontWeight={600}>{title}</Text>
               <Spacer />
              <AlertDialog.CloseButton right={0} />
              </HStack>
            </AlertDialog.Header>
            ...
            <AlertDialog.Footer borderColor={'primary.bdColor'} flexDirection={'column'} display={'flex'}  justifyContent={'flex-start'} p={0} bg={'transparent'}>
              <Button.Group variant={'unstyled'} space={0}>
                {btnRightText && <Center w="50%">
                  <Button  _text={{fontSize: '2xl',textTransform: 'uppercase'}} fontWeight={500} bg={'none'} w="100%" rounded={0} variant="unstyled"  onPress={btnRightFunc}>
                    {btnRightText}
                  </Button>
                </Center>
                }
              {btnLeftText  && <Container borderLeftWidth={1} borderLeftColor={'primary.bdColor'} w="50%">
                <Button _text={{fontSize: '2xl',textTransform: 'uppercase'}}  bg={'none'} w="100%" rounded={0} variant="unstyled"  fontWeight={500} onPress={btnLeftFunc} ref={cancelRef}>
                  {btnLeftText}
                </Button>
              </Container>
              }
