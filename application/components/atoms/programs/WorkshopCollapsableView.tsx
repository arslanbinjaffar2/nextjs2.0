    ...
    <>
      ...
      {!open && <>
        {program.workshop_programs?.map((workshop_program: Program, i: number) =>
            <RectangleDetailView workshop={true} key={i} section={section} speaker={speaker} program={workshop_program} k={i} border={border || program.workshop_programs?.length !== (i + 1)} />
            )}
        </>}
    </>
  )
