...
return (
    <Pressable isDisabled={in_array(`program-fav-${program_id}`,processing)} onPress={() => toggleFav()}>
        ...
        <Spinner width={28} height={28} color={calculatedValue ? 'secondary.500' : 'primary.text'} size="sm"  />
    ):(
        <Icon size="xl" as={AntDesign} name={calculatedValue ? 'heart' : 'hearto'} color={calculatedValue ? 'secondary.500' : 'primary.text'} />
    )}
    </Pressable>
)}
