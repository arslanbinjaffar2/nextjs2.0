          ...
          <Center>
            {post.attendee.id === response?.data?.user?.id && (
              <HStack px={3} w={'100%'} justifyContent={'flex-end'} space="3" alignItems="center">
                ...
                >
                  <Menu.Item  _text={{color: `${colorText(event.settings?.app_text_mode || 'primary.boxsolidtext')}`}} _focus={{ bg: '' }} _hover={{ bg: 'primary.500' }} onPress={() => { push(`/${event.url}/social_wall/edit/${post.id}`) }}>{event?.labels?.GENERAL_EDIT}</Menu.Item>
                  <Menu.Item  _text={{color: `${colorText(event.settings?.app_text_mode || 'primary.boxsolidtext')}`}} _focus={{ bg: '' }} _hover={{ bg: 'primary.500' }} onPress={() => { deletePost() }}>{event?.labels?.GENERAL_DELETE}</Menu.Item>
                </Menu>
              </HStack>
