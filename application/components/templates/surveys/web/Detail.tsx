        ...
        const submitedData:SubmittedQuestion[] | undefined = detail?.questions.map((q)=>{
            ...
            if(q.question_type === 'single' || q.question_type === 'multiple' || q.question_type === 'dropdown' || q.question_type === 'matrix'){
              ...
              if(q.question_type === 'single'){
                ...
                answeredQuestion['answers'] = (formData[q.id] !== undefined && Object.keys(formData[q.id].answer).length > 0) ? Object.keys(formData[q.id].answer).reduce((ack:any, i)=>([...ack, {id: `${formData[q.id].answer[i]}_${i}`}]), []) : [];
              }
            }
            else{
