...
import { HdSettings } from 'application/models/hd/Hd';
...
const Detail = () => {
    ...
    const { hdDetials, hdSettings, FetchProgramDetail, FetchTabDetails,  SubmitHd, SubmitHdLike, HdRecentPopularSocketUpdate, HdSort} = UseHdService();
    ...
    const onSubmit = ( ) => {
        ...
        if(hdSettings?.enable_paragraph_number == 1 && (paragraph == null || paragraph == 0)){
            setError('Please select a paragraph');
            return;
        }
        
        if(hdSettings?.line_number == 1 && hdSettings?.enable_line_number == 1 && (lineNumber == '')){
            setError('Please add line number');
            return;
        }


        const sp = hdDetials?.speakers?.find((sp)=>(sp.id == speaker));
        const pg = hdDetials?.paragraph?.find((sp)=>(sp.id == paragraph));
        ...
        const postData =  {
            ...
            answered: 0,
            allLanguages: JSON.stringify(hdDetials.all_languages),
        ...
        in_array('hd-detail', processing) ? (
            ...
            <Container overflow="hidden" mb="4" maxW="100%" w="100%">
                ...
                <Box w="100%"  py="3">
                    <HStack width={"100%"} pl="30px" alignItems="center" minH="55px" space={0} justifyContent="flex-start">
                        <Box position="absolute" left="0" top="0" w="15px">
                            <ZStack>
                            {hdDetials?.program_detail && hdDetials?.program_detail?.tracks?.length > 0 && hdDetials?.program_detail?.tracks.map((track: any, i: number) =>
                        ...
                        <HStack pt="0" w="100%" space="5" alignItems="center" justifyContent="space-between">
                            ...
                            {hdDetials?.program_detail?.info?.topic}
                            </Text>
                            </VStack>
                        </HStack>
                    </HStack>
                ...
                <Box w="100%">
                    ...
                    {hdDetials?.speakers?.length > 0 && <HStack pl="6"  w="100%" bg="primary.box" mb="3" alignItems="center">
                    ...
                    >
                        {hdDetials?.speakers?.map((speaker, i)=>(
                            <Select.Item label={`${speaker?.attendee?.first_name} ${speaker?.attendee?.last_name}`} value={`${speaker?.id}`} />
                        ))}
                    </Select>
                    
                    </HStack>}
                    {hdDetials?.paragraph?.length > 0 && <HStack pl="6"  w="100%" bg="primary.box" mb="3" alignItems="center">
                    ...
                    >
                        {hdDetials?.paragraph?.map((pg, i)=>(
                            <Select.Item label={`${pg?.heading}`} value={`${pg.id}`} />
                        ))}
                    </Select>
                    
                    </HStack>}
                    {hdSettings?.line_number == 1 && <HStack pl="6" pr={"6"}  w="100%" height={'40px'} bg="primary.box" mb="3" alignItems="center">
                ...
                {hdSettings?.hd_tabs == 1 && <Box w="100%">
                    ...
                    <Box mb="10" px="5" w="100%" position="relative">
                        ...
                        {!loading && <>
                            ...
                            <VStack w="100%" space="3">
                                {tab === 'popular' &&
                                  hdDetials?.popular_questions?.map((question,i)=>(
                                    <>
                                ...
                                {tab === 'recent' &&
                                  hdDetials?.recent_questions?.map((question,i)=>(
                                    <>
                                    <HStack w="100%" space="3" alignItems="center">
                                    <Avatar
                                        size="md"
                                ...
                                {tab === 'archive' &&
                                  hdDetials?.archived_questions?.map((question,i)=>(
                                    <>
                                    <HStack w="100%" space="3" alignItems="center">
                                    <Avatar
                                        size="md"
                                ...
                                {tab === 'my_question' &&
                                  hdDetials?.my_questions?.map((question,i)=>(
                                    <>
                                    <HStack w="100%" space="3" alignItems="center">
                                    <Avatar
                                        size="md"
