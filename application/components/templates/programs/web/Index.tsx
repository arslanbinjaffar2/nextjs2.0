...
const Index = () => {
    ...
    return (
        <>
            ...
            {(in_array('programs', processing) || in_array('tracks', processing)) && page === 1 ? null : <HStack flexWrap={'wrap'} mb="3" overflow={'hidden'} rounded={8} space={1} justifyContent="center" w="100%">
                {(event?.agenda_settings?.agenda_list == 1 || event?.agenda_settings?.agenda_tab == 1) && <ButtonElement  bg={in_array(tab, ['program', 'track-program']) ? 'primary.boxbutton' : 'primary.box'} onPress={() => {ResetTracks();setTab('program')}}>{modules?.find((module)=>(module.alias == 'agendas'))?.name ?? 'Program'}cccccccccccccccccccccccccccccccccc</ButtonElement>}
