                    ...
                    <Container pt="2" maxW="100%" w="100%">
                        ...
                        {tab === 'pending' &&  (
                            <Box overflow="hidden" bg="primary.box" w="100%" rounded="lg">
                                    {Object.keys(polls).reduce((ack:Poll[] | [], key) => ( [...ack, ...polls[key]]), []).map((poll:Poll)=>(
                        ...
                        {tab === 'completed' && (
                                <Box overflow="hidden" bg="primary.box" w="100%" rounded="lg">
                                    {Object.keys(completed_polls).reduce((ack:Poll[] | [], key) => ( [...ack, ...completed_polls[key]]), []).map((poll:Poll)=>(
                                        <RectangleView key={poll.id} poll={poll} />
                                    ))}
                                    <Divider h="100px" bg="transparent" />
                                </Box>
