    ...
    <>
      <Container maxW="100%" w="100%">
        <HStack w="100%" alignItems="flex-start" space="5">
          <Center overflow="hidden" alignItems="flex-start" w="100%" maxW={width! > 1200 ? '265px' : '265px'}>
            <Pressable onPress={() => { router.asPath.includes('/auth/gdpr') ? null : router.push(`/${event.url}/dashboard`) }}>
             <Image
                  alt='logo' mb={{ base: 5, md: 0 ,lg: 0,xl: 10 }} resizeMode='contain' source={{ uri: event.settings?.app_header_logo ? `${_env.eventcenter_base_url}/assets/event/branding/${event.settings.app_header_logo}`
          ...
          {width > 1200 && <Center w="100%" maxW={width! > 1200 ? '600px' : 'calc(100% - 200px)'}>
